...
台股 & 美股每日盤後自動分析，透過 LINE Messaging API 推播報告至手機。
Windows 工作排程器定時觸發，無需長時間掛載程序。

## 功能

- 每日自動抓取收盤價、成交量
- 計算 MA5 / MA20 / MA60、RSI、MACD
- 顯示 EPS、本益比、市值
- 附近三則最新新聞標題
- 漲跌超過門檻自動加警示 emoji
- 訊息超過字數限制時自動拆分發送
...
```
填入 LINE Messaging API 的 Channel Access Token 與你的 LINE User ID。
...
| `python stock_tracker.py test` | 同 once（測試用別名） |

## 專案結構

```
stock_agent/
├── stock_tracker.py   # 主程式：模式切換 + 排程
├── config.py          # 設定讀取（.env）
├── fetcher.py         # yfinance 數據抓取
├── indicators.py      # MA / RSI / MACD 計算
├── notifier.py        # LINE 訊息格式化與發送
├── run_tw.bat         # 台股執行腳本（Task Scheduler 用）
├── run_us.bat         # 美股執行腳本（Task Scheduler 用）
├── .env               # 機密設定（不進 git）
└── .env.example       # 設定範本
```
...
```ini
# LINE Messaging API（https://developers.line.biz/ 申請）
LINE_CHANNEL_TOKEN=你的_Channel_Access_Token
LINE_USER_ID=你的_User_ID            # 格式：Uxxxxxxxxxx
